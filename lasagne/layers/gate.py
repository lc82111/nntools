...
from lasagne.layers import Layer, SliceLayer, MergeLayer, DenseLayer, TransposedDenseLayer, ElemwiseMergeLayer, InputLayer, DropoutLayer, helper, GaussianNoiseLayer
import lasagne
...
class FactorGateLayer(ElemwiseMergeLayer):
    ...
    W_x,y,m b_m  : Theano shared variable or callable. 
...
class PGPLayer(Layer):
    ...
    def __init__(self, incoming,
        ...
        self.l_m   = FactorGateLayer([self.l_x_i, self.l_y_i], num_factors=self.num_factors, num_maps=self.num_maps, W_x=W_x, W_y=W_y, W_m=W_m, b_m=b_m)
...
def build_GAE(batch_size, frame_dim, num_fac=200, num_maps=100, noise=0.3):
    l_x_i = InputLayer(shape=(batch_size, frame_dim), name='l_x_i')
    l_x   = DropoutLayer(l_x_i, p=noise)
    l_x   = DenseLayer(l_x, num_units=num_fac, nonlinearity=None, W=init.GlorotNormal())

    l_y_i = InputLayer(shape=(batch_size, frame_dim), name='l_y_i')
    l_y   = DropoutLayer(l_y_i, p=noise)
    l_y   = DenseLayer(l_y, num_units=num_fac, nonlinearity=None, W=init.GlorotNormal())

    l_m   = GateTriangleLayer([l_x, l_y], num_units=num_maps, name='l_m', nonlinearity=nonlinearities.sigmoid) 
    l_m   = TransposedDenseLayer(l_m, W=l_m.W.T, num_units=num_fac, b=None)
    l_x_r = TransposedGateTriangleLayer([l_y, l_m], num_units=frame_dim, W=l_x.W.T, name='l_x_r', nonlinearity=None)
    l_y_r = TransposedGateTriangleLayer([l_x, l_m], num_units=frame_dim, W=l_y.W.T, name='l_y_r', nonlinearity=None)

    #l_m   = FactorGateLayer([l_x,l_y], num_factors=num_fac, num_maps=num_maps)

    #l_x_r = FactorGateLayer([l_m, l_y], num_factors=num_fac, num_maps=frame_dim, W_x=l_m.W.T, W_y=l_m.W_y, W=l_m.W_x.T, name='l_x_r', nonlinearity=None)
    #l_y_r = FactorGateLayer([l_m, l_x], num_factors=num_fac, num_maps=frame_dim, W_x=l_m.W.T, W_y=l_m.W_x, W=l_m.W_y.T, name='l_y_r', nonlinearity=None)

    return l_x_i, l_y_i, l_x_r, l_y_r

def create_iter_functions(dataset, l_x_i, l_y_i, l_x_r, l_y_r, batch_size=400, momentum=0.9):
    """Create functions for training, validation and testing to iterate one
       epoch.
    """
    X_batch = T.matrix('X')
...
def train(iter_funcs, dataset, learning_rate, batch_size=400):
...
def main_GAE():
    # load data
    dataset = make_sinwave()

    # build GAE model
    l_x_i, l_y_i, l_x_r, l_y_r = build_GAE(batch_size=400, frame_dim=dataset['frame_dim'])

    # build iter funcs
    iter_funcs = create_iter_functions(dataset, l_x_i, l_y_i, l_x_r, l_y_r)
